...
let word = window.localStorage.getItem("secondWord")
console.log(word)
let word1 =[]
    ...
    for (let index = 0; index < document.querySelectorAll("h2").length; index++) {
        ...
        if (document.querySelectorAll("h2")[index].getAttribute("class") == null || document.querySelectorAll("h2")[index].getAttribute("class") == "null") {
            document.querySelectorAll("h2")[index].onclick = function () {
                ...
                window.localStorage.setItem("rounds", Number(window.localStorage.getItem("rounds") -1))
...
//    console.log(window.localStorage.getItem("word"))
    if (l == window.localStorage.getItem("word").length) {
        window.localStorage.setItem("rounds", Number(window.localStorage.getItem("rounds") -1))
        if (window.localStorage.getItem("secondScore") != null) {
            
            console.log("fuck")
             window.localStorage.setItem("secondScore", `${Number(window.localStorage.getItem("secondScore")) + 50}`)
         } else {
             window.localStorage.setItem("secondScore", 50)
         }
         setTimeout(() => {
